...
func FormatGameInfo(gi *game.GameInfo) string {
	...
	if gi.GT == game.GTcrash {
		fmt.Fprintf(&b, "'%s' %s crash game", gi.Name, gi.Prov)
	} else if gi.GT == game.GTcard {
		fmt.Fprintf(&b, "'%s' %s card game", gi.Name, gi.Prov)
	} else if gi.GT == game.GTfish {
		fmt.Fprintf(&b, "'%s' %s %d cell fishing game", gi.Name, gi.Prov, gi.SX)
	} else if gi.SN > 0 {
