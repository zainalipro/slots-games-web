...
// Withdraws coins from user wallet to external payment (real withdrawal).
func ApiPropsWithdraw(c *gin.Context) {
	var err error
	var ok bool
	var arg struct {
		XMLName xml.Name `json:"-" yaml:"-" xml:"arg"`
	...
	if err = c.ShouldBind(&arg); err != nil {
		Ret400(c, AEC_prop_withdraw_nobind, err)
		return
	}

	var club *Club
	if club, ok = Clubs.Get(arg.CID); !ok {
		Ret404(c, AEC_prop_withdraw_noclub, ErrNoClub)
		return
	}
	_ = club
	...
	var user *User
	if user, ok = Users.Get(arg.UID); !ok {
		Ret404(c, AEC_prop_withdraw_nouser, ErrNoUser)
		return
	}

	var admin, al = MustAdmin(c, arg.CID)
	if al&ALbooker == 0 {
		Ret403(c, AEC_prop_withdraw_noaccess, ErrNoAccess)
		return
	}

	var props *Props
	if props, ok = user.props.Get(arg.CID); !ok {
		Ret500(c, AEC_prop_withdraw_noprops, ErrNoProps)
		return
	}
	if props.Wallet < arg.Sum {
		Ret403(c, AEC_prop_withdraw_nomoney, ErrNoMoney)
		return
	}

	// Positive sum = money leaves player wallet, goes to bank (like a game loss)
	if Cfg.ClubUpdateBuffer > 1 {
		go BankBat[arg.CID].Put(cfg.XormStorage, arg.UID, arg.Sum)
	} else if err = BankBat[arg.CID].Put(cfg.XormStorage, arg.UID, arg.Sum); err != nil {
		Ret500(c, AEC_prop_withdraw_sql, err)
		return
	}

	// Also log the withdrawal as a wallet transaction
	if Cfg.ClubInsertBuffer > 1 {
		go BankBat[arg.CID].Add(cfg.XormStorage, arg.UID, admin.UID, props.Wallet-arg.Sum, -arg.Sum)
	} else if err = BankBat[arg.CID].Add(cfg.XormStorage, arg.UID, admin.UID, props.Wallet-arg.Sum, -arg.Sum); err != nil {
		Ret500(c, AEC_prop_withdraw_sql, err)
